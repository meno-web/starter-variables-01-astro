---
description: For every section component referenced by pages/<slug>.json, decide whether it needs interactivity (mobile menu, dropdown, tabs, accordion, modal trigger, carousel, sticky header, smooth-scroll, copy-to-clipboard, form behavior) and, if so, add the right data attributes + JS file. Operates in parallel: one sub-agent per section. Wires cross-component interactions (e.g. trigger button -> Modal) via CustomEvents.
...
Walk through every section component used by `pages/<slug>.json` and add the JavaScript interactivity each one needs.
...
1. **Parse `$ARGUMENTS`.** The first token is the page slug; an optional `--port=N` token sets the Studio editor port (useful when multiple Meno projects are running and this one is on 3001/3002/...).
   - Reject (halt with one-line error) if the slug is empty, contains a slash, or ends in `.json`.
   - If `--port` is present and not an integer in 1024–65535, drop it and fall back to auto-detection (don't halt).

2. **Validate the page exists.** Confirm `pages/<slug>.json` is readable. If missing, halt with a one-line error — do NOT prompt for alternatives.
...
- This skill assumes `/split-page` (or its equivalent) has already run — `pages/<slug>.json` should already be a thin shell of component refs.
...
- Each section worker takes its own screenshot (selector `[data-component-context="<Name>"][data-component-root="true"]`) and reasons from screenshot + JSON together. Screenshots are NOT shared across workers — one image per fresh sub-agent context.
